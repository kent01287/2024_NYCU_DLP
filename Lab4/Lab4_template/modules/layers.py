...
class ResidualBlock(nn.Module):
    def __init__(self, in_ch: int, out_ch: int, stride=1):
        ...
        if in_ch != out_ch: #ResNet 34 用來調整image size 讓 x能與f(x)相加
...
class ConvFFN(nn.Module):
    def __init__(self, in_ch, slope=0.1, inplace=False):
        ...
        self.conv = nn.Conv2d(in_ch, internal_ch * 2, 1) #kernel size =1
        self.conv_out = nn.Conv2d(internal_ch, in_ch, 1) #kernel size =1
        self.relu = nn.LeakyReLU(negative_slope=slope, inplace=inplace) #允許負值以較小的斜率傳遞，而不是將負值全部置零。
        #輸入是negative時 slope 0.1

    def forward(self, x):
        identity = x
        x1, x2 = self.conv(x).chunk(2, 1) #分割成兩個大小 以第一個dim (channal) 進行分割
        out = x1 * self.relu(x2)
...
class DepthConv(nn.Module):
    ...
    def forward(self, x):
        identity = x
        if self.adaptor is not None: # 原本輸出 x 需要降維 才能與out 相加
